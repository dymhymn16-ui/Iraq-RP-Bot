...
const commands = [
  ...
  new SlashCommandBuilder().setName("toprich").setDescription("عرض قائمة أكثر الناس ثراءً وأغنى اللاعبين في السيرفر"),
  ...
  new SlashCommandBuilder()
    .setName("gangdeposit")
    .setDescription("إيداع أموال من محفظتك إلى بنك العصابة")
    .addIntegerOption(o => o.setName("amount").setDescription("المبلغ المراد إيداعه").setRequired(true).setMinValue(1)),

  new SlashCommandBuilder()
    .setName("gangwithdraw")
    .setDescription("سحب أموال من بنك العصابة إلى محفظتك (للزعيم أو النائب)")
    .addIntegerOption(o => o.setName("amount").setDescription("المبلغ المراد سحبه").setRequired(true).setMinValue(1)),

  new SlashCommandBuilder()
    .setName("gangstoreitem")
    .setDescription("تخزين الحشيش في مخزن العصابة")
    .addIntegerOption(o => o.setName("amount").setDescription("كمية الحشيش المراد تخزينها (كغ)").setRequired(true).setMinValue(1)),
  ...
  new SlashCommandBuilder()
    .setName("disbandgang")
    .setDescription("حل العصابة نهائياً (للزعيم فقط)"),
  ...
  setInterval(async () => {
    try {
      ...
      members.forEach(member => {
        ...
        // 👑 قائد الشرطة
        if (member.roles.cache.has(config.roles.policeLeader)) {
          salaryAmount = 30000;
          salaryName = "قائد الشرطة";

        // 🥷 نائب الشرطة
        } else if (member.roles.cache.has(config.roles.policeDeputy)) {
          salaryAmount = 20000;
          salaryName = "نائب الشرطة";

        // 👮 الشرطي
        } else if (member.roles.cache.has(config.roles.policeOfficer)) {
          salaryAmount = 15000;
          salaryName = "الشرطي";
        }
        ...
        if (salaryAmount > 0) {
          ...
          member.send(
            `💰 **تم إيداع راتبك!**\n\n` +
            `👤 الوظيفة: **${salaryName}**\n` +
            `💵 الراتب: **$${money(salaryAmount)}**\n` +
            `🏦 تم الإيداع في حسابك البنكي.`
          ).catch(() => {});
        }
      });
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    if (interaction.commandName === "toprich") {
      ...
      const embed = new EmbedBuilder()
        .setTitle("🏆 لوحة الشرف - أكثر الناس ثراءً وأغنى أثرياء المدينة")
    ...
    if (interaction.commandName === "gangdeposit") {
      const result = gangOf(db, id);
      if (!result) {
        const embed = new EmbedBuilder()
          .setTitle("❌ خطأ في العملية")
          .setDescription("عذراً، يجب أن تكون منضماً إلى عصابة لكي تتمكن من إيداع الأموال في بنك العصابة!")
          .setColor(0xff0000);
        return interaction.reply({ embeds: [embed], ephemeral: true });
      }
      ...
      if (uData.balance < amount) {
        const embed = new EmbedBuilder()
          ...
          .setDescription(`لا تمتلك هذا المبلغ في محفظتك الشخصية! رصيدك الحالي: **$${money(uData.balance)}**`)
          .setColor(0xff0000);
        return interaction.reply({ embeds: [embed], ephemeral: true });
      }

      uData.balance -= amount;
      result.gang.bank = (result.gang.bank || 0) + amount;
      saveData(db);

      const embed = new EmbedBuilder()
        .setTitle("🏦 إيداع ناجح في بنك العصابة")
        .setDescription(`تم إيداع مبلغ **$${money(amount)}** بنجاح في خزينة عصابة **${result.gang.name}**.\n\n💰 رصيد بنك العصابة الإجمالي: **$${money(result.gang.bank)}**`)
        .setColor(0x00ff00);
      return interaction.reply({ embeds: [embed], ephemeral: true });
    }

    if (interaction.commandName === "gangwithdraw") {
      const result = gangOf(db, id);
      if (!result) {
        const embed = new EmbedBuilder()
          .setTitle("❌ خطأ في العملية")
          .setDescription("عذراً، يجب أن تكون منضماً إلى عصابة لتنفيذ هذا الأمر!")
          .setColor(0xff0000);
        return interaction.reply({ embeds: [embed], ephemeral: true });
      }
      ...
      if (!gangManagement(db, id)) {
        const embed = new EmbedBuilder()
          .setTitle("❌ صلاحيات مرفوضة")
          .setDescription("أمر سحب الأموال من بنك العصابة مخصص لزعيم العصابة أو النائب فقط!")
          .setColor(0xff0000);
        return interaction.reply({ embeds: [embed], ephemeral: true });
      }

      const amount = interaction.options.getInteger("amount");
      if ((result.gang.bank || 0) < amount) {
        const embed = new EmbedBuilder()
          .setTitle("❌ البنك فارغ")
          .setDescription(`بنك العصابة لا يحتوي على هذا المبلغ! رصيد العصابة الحالي: **$${money(result.gang.bank)}**`)
          .setColor(0xff0000);
        return interaction.reply({ embeds: [embed], ephemeral: true });
      }
      ...
      const embed = new EmbedBuilder()
        .setTitle("💸 سحب ناجح من بنك العصابة")
        .setDescription(`تم سحب مبلغ **$${money(amount)}** من خزينة عصابة **${result.gang.name}** وإضافته إلى محفظتك الشخصية.`)
        .setColor(0xffa500);
      return interaction.reply({ embeds: [embed], ephemeral: true });
    }

    if (interaction.commandName === "gangstoreitem") {
      const result = gangOf(db, id);
      if (!result) {
        const embed = new EmbedBuilder()
          .setTitle("❌ خطأ في العملية")
      ...
      result.gang.storage.weed = (result.gang.storage.weed || 0) + amount;
      saveData(db);

      const embed = new EmbedBuilder()
        .setTitle("🌿 تخزين المواد المخدرة")
        .setDescription(`تم وضع وإيداع **${amount} كغ** من الحشيش في مخزن عصابة **${result.gang.name}** بنجاح.\n\n📦 إجمالي الحشيش بالمخزن: **${result.gang.storage.weed} كغ**`)
        .setColor(0x00ff00);
      return interaction.reply({ embeds: [embed], ephemeral: true });
    }
    ...
    if (interaction.commandName === "creategang") {
      if (police(member)) {
        return interaction.reply({ content: "❌ عذراً، لا يمكن لأعضاء الشرطة إنشاء عصابات!", ephemeral: true });
      }
      if (gangOf(db, id)) {
        return interaction.reply({ content: "❌ أنت منضم إلى عصابة مسبقاً ولا يمكنك إنشاء عصابة جديدة.", ephemeral: true });
      }
      ...
      return interaction.reply(`🔫 تم إنشاء عصابة **${name}** بنجاح.\n👑 الزعيم: ${interaction.user}`);
    }

    if (interaction.commandName === "disbandgang") {
      const result = gangOf(db, id);
      if (!result) {
        return interaction.reply({ content: "❌ أنت لست منضمًا إلى أي عصابة أساساً!", ephemeral: true });
      }
      if (result.gang.leader !== id) {
        return interaction.reply({ content: "❌ أمر حل العصابة مخصص لزعيم العصابة فقط!", ephemeral: true });
      }

      const gangData = result.gang;
      const gangId = result.id;

      for (const memberId of gangData.members) {
        try {
          const targetMember = await interaction.guild.members.fetch(memberId);
          if (targetMember) {
            await takeRole(targetMember, config.roles.gangLeader);
            await takeRole(targetMember, config.roles.gangDeputy);
            await takeRole(targetMember, config.roles.gangMember);
          }
        } catch (e) {}
      }

      delete db.gangs[gangId];
      saveData(db);

      return interaction.reply(`🗑️ تم حل عصابة **${gangData.name}** نهائياً وإزالة جميع الصلاحيات والأموال والمخزن الخاصة بها بناءً على طلب الزعيم.`);
    }
    ...
    if (interaction.commandName === "leavegang") {
      const result = gangOf(db, id);
      if (!result) return interaction.reply({ content: "❌ أنت لست بعصابة.", ephemeral: true });
      if (result.gang.leader === id) return interaction.reply({ content: "❌ الزعيم لا يستطيع المغادرة. استخدم أمر حل العصابة إذا أردت حذفها.", ephemeral: true });
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    if (interaction.customId.startsWith("rob_")) {
      ...
      const reward = Math.floor(Math.random() * 40000) + 10000;
      if (type === "store" || type === "bank") {
        const reward = Math.floor(Math.random() * 40000) + 100;
if (type === "store" || type === "bank") {
  result.gang.storage.stolen = reward;
}
...
if (type === "store") {
  successMessage =
    `🏪 **نجحت عملية سرقة المتجر!**\n\n` +
    `💰 الأموال المسروقة: **$${money(reward)}**\n` +
    `📦 تمت إضافة المسروقات إلى مخزن العصابة.`;

} else if (type === "bank") {
  successMessage =
    `🏦 **نجحت عملية سرقة البنك!**\n\n` +
    `💰 الأموال المسروقة: **$${money(reward)}**\n` +
    `📦 تمت إضافة المسروقات إلى مخزن العصابة.`;

} else if (type === "drugs") {
  successMessage =
    `🌿 **نجحت عملية صناعة الحشيش!**\n\n` +
    `📦 الكمية المنتجة: **${result.gang.storage.weed} كغ إجمالي بالمخزن**`;
}

// تحديث المهمة اليومية للعصابة
if (db.missions && db.missions[userId]) {

  const mission = db.missions[userId];
...
// =========================
// نظام الانضمام إلى الحرب
// =========================

if (interaction.customId.startsWith("war_join_police_")) {

  const warId = interaction.customId.replace(
    "war_join_police_",
    ""
  );
  ...
  if (!war || war.status !== "recruiting") {
    return interaction.reply({
      content: "❌ هذه الحرب لم تعد متاحة.",
      ephemeral: true
    });
  }

  if (!war.policeTeam) war.policeTeam = [];
  if (!war.gangTeam) war.gangTeam = [];

  const member = interaction.member;

  if (!police(member)) {
    return interaction.reply({
      content: "❌ هذا الفريق مخصص للشرطة فقط.",
      ephemeral: true
    });
  }

  if (war.policeTeam.includes(interaction.user.id)) {
    return interaction.reply({
      content: "❌ أنت منضم مسبقاً إلى فريق الشرطة.",
      ephemeral: true
    });
  }

  if (war.gangTeam.includes(interaction.user.id)) {
    return interaction.reply({
      content: "❌ أنت منضم مسبقاً إلى الطرف الآخر.",
      ephemeral: true
    });
  }

  if (war.policeTeam.length >= 6) {
    return interaction.reply({
      content: "❌ فريق الشرطة مكتمل.",
      ephemeral: true
    });
  }
  ...
  return interaction.reply({
    content: "🚔 تم انضمامك إلى فريق الشرطة في الحرب!",
    ephemeral: true
  });
}
...
// =========================
// انضمام العصابة إلى الحرب
// =========================

if (interaction.customId.startsWith("war_join_gang_")) {
  ...
  if (!result) {
    return interaction.reply({
      content: "❌ يجب أن تكون عضواً في عصابة.",
      ephemeral: true
    });
  }

  if (result.id !== war.gangId) {
    return interaction.reply({
      content: "❌ أنت لست عضواً في العصابة المشاركة بالحرب.",
      ephemeral: true
    });
  }
  ...
  if (war.gangTeam.includes(interaction.user.id)) {
    return interaction.reply({
      content: "❌ أنت منضم مسبقاً إلى فريق العصابة.",
      ephemeral: true
    });
  }
  ...
  if (war.gangTeam.length >= 6) {
    return interaction.reply({
      content: "❌ فريق العصابة مكتمل.",
      ephemeral: true
    });
  }
  ...
  return interaction.reply({
    content: "⚔️ تم انضمامك إلى فريق العصابة في الحرب!",
    ephemeral: true
  });
}
...
if (interaction.customId.startsWith("war_p_")) {
  ...
  if (!gangId || !targetId) {
    return interaction.reply({
      content: "❌ بيانات الهجوم غير صحيحة.",
      ephemeral: true
    });
  }
  ...
  const activeWar = Object.entries(db.wars).find(
    ([id, war]) =>
      war.status === "active" &&
      war.gangId === gangId
  );
  ...
  if (!activeWar) {
    return interaction.reply({
      content: "❌ لا توجد حرب نشطة.",
      ephemeral: true
    });
  }

  const war = activeWar[1];

  if (!war.gangTeam) war.gangTeam = [];

  if (!war.gangTeam.includes(interaction.user.id)) {
    return interaction.reply({
      content: "❌ هذا الزر مخصص لأعضاء العصابة.",
      ephemeral: true
    });
  }
  ...
  if (interaction.user.id === targetId) {
    return interaction.reply({
      content: "❌ لا يمكنك اختيار نفسك.",
      ephemeral: true
    });
  }

  if (!war.gangScore) war.gangScore = 0;
  if (!war.policeScore) war.policeScore = 0;

  const points = Math.floor(Math.random() * 5) + 1;

  war.gangScore += points;
...
if (interaction.customId.startsWith("war_g_")) {
...
if (interaction.customId.startsWith("war_manual_end_")) {

  if (
    !policeLeader(interaction.member) &&
    !owner(interaction.member)
  ) {
    return interaction.reply({
      content:
        "❌ إنهاء الحرب مخصص لقائد الشرطة أو مالك السيرفر.",
      ephemeral: true
    });
  }

  const gangId = interaction.customId.replace(
    "war_manual_end_",
    ""
  );
  ...
  const warEntry = Object.entries(db.wars).find(
    ([id, war]) =>
      war.gangId === gangId &&
      war.status === "active"
  );
  ...
  const war = warEntry[1];

  war.status = "ended";

  const policeScore = Number(war.policeScore) || 0;
  const gangScore = Number(war.gangScore) || 0;

  let winner = "";

  if (policeScore > gangScore) {
    winner = "🚔 الشرطة";
  } else if (gangScore > policeScore) {
    winner = "🔫 العصابة";
  } else {
    winner = "🤝 تعادل";
  }

  const gang = db.gangs?.[war.gangId];

  if (gang) {

    if (policeScore > gangScore) {
      gang.losses = (gang.losses || 0) + 1;
    }

    if (gangScore > policeScore) {
      gang.wins = (gang.wins || 0) + 1;
    }
  }
  ...
  const embed = new EmbedBuilder()
    .setTitle("🏁 انتهت الحرب")
    .setDescription(
      `⚔️ **النتيجة النهائية**\n\n` +
      `🚔 الشرطة: **${policeScore} نقطة**\n` +
      `🔫 العصابة: **${gangScore} نقطة**\n\n` +
      `🏆 الفائز: **${winner}**`
    )
    .setColor(0xffd700)
    .setTimestamp();
      ...
      if (now >= Number(jailData.ends)) {
        ...
        for (const guild of client.guilds.cache.values()) {
          ...
          try {
            ...
            if (Array.isArray(jailData.roles)) {

              const validRoles = jailData.roles.filter(
                roleId => guild.roles.cache.has(roleId)
              );

              await member.roles.set(validRoles).catch(() => {});
            }

            member.send(
              "🎉 تم انتهاء مدة سجنك!\n\n" +
              "✅ تمت إزالة رتبة المسجون.\n" +
              "🔄 تمت إعادة رتبك السابقة."
            ).catch(() => {});
