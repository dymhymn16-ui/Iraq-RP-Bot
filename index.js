...
// =========================
// جدول مناطق اللعبة وتكوين أحداث الحروب التلقائية
// =========================
const WAR_ZONES = [
...
// =========================
// نظام الرواتب التلقائية في الخلفية (حسب الآيديات والمبالغ المطلوبة)
// =========================
  ...
  setInterval(async () => {
    try {
      ...
      members.forEach(member => {
        ...
        // قائد الشرطة (1541482535454777456) -> راتب 40000
        if (member.roles.cache.has("1541482535454777456")) {
          salaryAmount = 40000;
        }
        // نائب الشرطه (1541482645009989642) -> راتب 30000
        else if (member.roles.cache.has("1541482645009989642")) {
          salaryAmount = 30000;
        }
        // الشرطي (1541482824492650576) -> راتب 17000
        else if (member.roles.cache.has("1541482824492650576")) {
          salaryAmount = 17000;
        }
        ...
        if (salaryAmount > 0) {
          ...
          member.send(`💰 تم إيداع راتبك الحكومي بقيمة **$${money(salaryAmount)}** في حسابك البنكي تلقائياً!`).catch(() => {});
        }
      });
...
function updateActiveWarMessage(message, warData, gangName, clientInstance) {
  // توليد أزرار بأسماء وأدوار أعضاء الشرطة وأعضاء العصابة المشاركين في الحرب
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    if (interaction.customId.startsWith("war_")) {
      ...
      // نظام هجوم الحرب مع نقاط عشوائية (من 1 إلى 20) لجميع الرتب (قائد، نائب، شرطي، عضو عصابة)
      if (action === "p" || action === "g") {
        ...
        if (action === "p") {
          ...
          const pts = Math.floor(Math.random() * 20) + 1; // نقاط عشوائية من 1 إلى 20
