...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    if (interaction.customId.startsWith("rob_")) {
      ...
      db.cooldowns[userCdKey] = now;
      if (!result.gang.storage) result.gang.storage = { weed: 0, stolen: 0
      saveData(db);
      ...
      if (type === "store") {
        successMessage =
          ...
          `📦 تمت إضافة مسروقات إلى مخزن العصابة.`;
    ...
    if (interaction.customId.startsWith("war_p_")) {
      ...
      const wars = Object.entries(db.wars);

      const activeWar = wars.find(([id, war]) =>
        war.status === "active" &&
        war.gangId === parts[2]
      );
    ...
    if (interaction.customId.startsWith("war_manual_end_")) {
      ...
      if (!policeLeader(interaction.member) && !owner(interaction.member)) {
        return interaction.reply({
          content: "❌ إنهاء الحرب مخصص لقائد الشرطة أو مالك السيرفر.",
          ephemeral: true
        });
      }
      ...
      if (war.policeScore > war.gangScore) {
        winner = "🚔 الشرطة";
      } else if (war.gangScore > war.policeScore) {
        winner = "🔫 العصابة";
      } else {
        winner = "🤝 تعادل";
      }
      ...
      if (gang) {
        ...
        if (war.gangScore > war.policeScore) {
