...
const commands = [
  ...
  // الأوامر المالية الجديدة المطلوبة
  new SlashCommandBuilder()
    .setName("deposit")
...
// =========================
// جدول مناطق اللعبة وتكوين أكلده الحروب التلقائية
// =========================
const WAR_ZONES = [
...
// دالة فحص وتفعيل الحروب التلقائية كل دقيقة (الأحد إلى الخميس في تمام الساعة 8:00 مساءً)
function setupAutoWars(clientInstance) {
  setInterval(async () => {
    const now = new Date();
    const day = now.getDay(); // 0 الأحد، 1 الإثنين، 2 الثلاثاء، 3 الأربعاء، 4 الخميس، 5 الجمعة، 6 السبت
    const hours = now.getHours();
    const minutes = now.getMinutes();

    // من الأحد (0) إلى الخميس (4) وعند الساعة 8:00 مساءً (20:00) والدقيقة 0
    if (day >= 0 && day <= 4 && hours === 20 && minutes === 0) {
      ...
      // اختيار عصابة عشوائية لهذه الجولة
      ...
      // اختيار منطقة عشوائية أو حسب اليوم
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          ...
          `- زر الشرطة مخصص لعناصر الشرطة فقط، وزر العصابة مخصص لأعضاء العصابة المستهدفة فقط!\n\n` +
        ...
        try {
          ...
          const updatedEmbed = new EmbedBuilder()
            ...
            .setDescription(
              ...
              `📋 **فريق العصابة (${currentWar.gangTeam.length}/6):** ${currentWar.gangTeam.map(x => `<@${x}>`).join(", ") || "لا يوجد"}`
            )
            .setColor(0xffa500);
        ...
        try {
          ...
          if (currentWar.policeTeam.length < 4 || currentWar.gangTeam.length < 4) {
            ...
            const cancelEmbed = new EmbedBuilder()
              ...
              .setDescription(`لم يتم استيفاء الحد الأدنى للمشاركين (4 أشخاص لكل طرف).\nالشرطة: (${currentWar.policeTeam.length}), العصابة: (${currentWar.gangTeam.length}).`)
              .setColor(0x808080);
          ...
          updateActiveWarMessage(response, currentWar, gangData.name, clientInstance, guild => guild);
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    // الأوامر المالية الجديدة (إيداع، سحب، تحويل)
    ...
    // 4. سوق العصابات السوداء (/gangshop)
    ...
    // 5. شراء من السوق السوداء (/gangbuy)
...
function updateActiveWarMessage(message, warData, gangName, clientInstance, guildFetcher) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `🔥 المعركة قائمة الآن وستستمر لمدة **10 دقائق**!\n- يمكن لكل مشارك الهجوم على خصمه كل **5 ثوانٍ** باستخدام الأزر أدناه بأسمائهم.\n\n` +
  ...
  try {
    ...
    if (interaction.customId.startsWith("war_")) {
      ...
      if (action === "join") {
        ...
        if (teamType === "police") {
          // منع العصابة تماماً من الضغط على زر الشرطة
          if (!police(interaction.member)) return interaction.reply({ content: "❌ هذا الفريق مخصص للشرطة فقط ولا يمكن لأعضاء العصابات الانضمام إليه.", ephemeral: true });
        ...
        if (teamType === "gang") {
          ...
          // منع الشرطة تماماً من الضغط على زر العصابة والعكس
          if (!userGang || userGang.id !== war.gangId)
            return interaction.reply({ content: "❌ هذا الزر مخصص فقط لأعضاء العصابة المستهدفة المحددة في الحرب!", ephemeral: true });
