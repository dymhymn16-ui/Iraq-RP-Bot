...
// خريطة لتخزين وقت آخر ضربة لكل مستخدم (Cooldown) لمدة 5 ثوانٍ
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o =>
      o.setName("gang")
       .setDescription("اسم أو معرف العصابة المستهدفة")
       .setRequired(true)),
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    // =========================
    // أمر الحرب (WAR) المحدث بنظام مرحلة التقديم (60 ثانية)
    // =========================
    if (interaction.commandName === "war") {
      ...
      if (!foundGang)
        return interaction.reply({ content: "❌ لم يتم العثور على العصابة المطلوبة.", ephemeral: true });
      ...
      db.wars[warId] = {
        ...
        policeScore: 0,
        gangScore: 0,
        policeTeam: [], // الأعضاء المسجلين من الشرطة
        gangTeam: [],   // الأعضاء المسجلين من العصابة
        participants: {},
        status: "recruiting", // مرحلة تلقي المشاركات
        startedAt: Date.now()
      };
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          ...
          .setLabel("⚔️ قبول الحرب (قائد العصابة)")
          .setStyle(ButtonStyle.Danger),
        new ButtonBuilder()
          .setCustomId(`war_join_police_${warId}`)
          .setLabel("🚔 الانضمام للشرطة (0/6)")
          .setStyle(ButtonStyle.Primary),
        new ButtonBuilder()
          .setCustomId(`war_join_gang_${warId}`)
          .setLabel(`🔫 الانضمام للعصابة (0/6)`)
          .setStyle(ButtonStyle.Success)
      );
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          ...
          `⏳ **مرحلة التسجيل مفتوحة لمدة 60 ثانية:**\n` +
      ...
      // مؤقت مدته 60 ثانية لتلقي المشاركات وبدء الحرب تلقائياً إذا توفرت الشروط
      setTimeout(async () => {
        try {
          ...
          // التحقق من الشروط (الحد الأدنى 4 أشخاص لكل فريق)
          ...
          // بدء الحرب رسمياً وتغيير الأزرار لأزرار الهجوم المباشر
          currentWar.status = "active";
          currentWar.warStartTime = Date.now();
          saveData(currentDb);

          // توليد أزرار الهجوم التفاعلية لكل فريق
          updateActiveWarMessage(response, currentWar, gangData.name);

          // مؤقت مدته 10 دقائق لإنهاء الحرب تلقائياً
          setTimeout(async () => {
...
// دالة مساعدة لتحديث رسالة الحرب النشطة بأزرار استهداف الأعضاء
...
// دالة الإنهاء التلقائي بعد 10 دقائق
async function endWarAutomatically(clientInstance, warId, gangName) {
  try {
    ...
    const endEmbed = new EmbedBuilder()
      .setTitle("🏁 انتهت المعركة (مرت 10 دقائق)!")
      .setDescription(
        `🏆 **المنتصر:** ${winner}\n\n` +
        `📊 **النتيجة النهائية:**\n` +
    ...
    // يمكن محاولة تعديل الرسالة إذا تم تخزين قناة ومعرف الرسالة، أو تركها بنظام الزر اليدوي
  } catch (e) {
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    // =========================
    // أزرار تفاعلات حرب الشرطة والعصابة (القبول، الانضمام، الهجوم، الإنهاء)
    // =========================
    if (interaction.customId.startsWith("war_")) {
      ...
      if (action === "accept") {
        ...
        if (!war || war.status !== "recruiting")
          return interaction.reply({ content: "❌ الحرب منتهية أو غير موجودة.", ephemeral: true });
      ...
      if (action === "join" && parts[2] === "police") {
        ...
        const war = db.wars[warId];
        if (!war || war.status !== "recruiting")
          return interaction.reply({ content: "❌ انتهى وقت التسجيل.", ephemeral: true });

        if (!police(interaction.member))
          return interaction.reply({ content: "❌ هذا الفريق مخصص للشرطة فقط.", ephemeral: true });

        if (war.policeTeam.includes(interaction.user.id))
          return.reply({ content: "❌ أنت مسجل مسبقاً في فريق الشرطة.", ephemeral: true });

        if (war.policeTeam.length >= 6)
          return interaction.reply({ content: "❌ اكتمل العدد الأقصى لفريق الشرطة (6 أشخاص).", ephemeral: true });
      ...
      if (action === "join" && parts[2] === "gang") {
        ...
        if (war.gangTeam.includes(interaction.user.id))
          return interaction.reply({ content: "❌ أنت مسجل مسبقاً في فريق العصابة.", ephemeral: true });

        if (war.gangTeam.length >= 6)
          return interaction.reply({ content: "❌ اكتمل العدد الأقصى لفريق العصابة (6 أشخاص).", ephemeral: true });
      ...
      // 4. هجوم الشرطة أو العصابة عبر الأزرار التفاعلية
      if (action === "p" || action === "g") {
        ...
        // التحقق من نظام التبريد (Cooldown) 5 ثوانٍ
        const lastHit = cooldowns.get(userId) || 0;
        ...
        if (!warEntry)
          return interaction.reply({ content: "❌ لا توجد حرب نشطة حالياً.", ephemeral: true });

        const gangData = db.gangs[gangId];
        const gangName = gangData ? gangData.name : "العصابة";

        warEntry.participants ??= {};

        // إذا كان الضاغط شرطياً يهاجم العصابة
        if (action === "p") {
          ...
          warEntry.participants[userId] ??= { name: userName, team: "الشرطة", points: 0 };
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `💥 قام الشرطي ${interaction.user} بضربة ناجحة ضد <@${targetId}> (+${pts} نقطة)!\n\n` +
              `📊 **النتيجة الحالية:**\n` +
        ...
        // إذا كان الضاغط من العصابة يهاجم الشرطة
        if (action === "g") {
          ...
          warEntry.participants[userId] ??= { name: userName, team: gangName, points: 0 };
          warEntry.participants[userId].points += pts;
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `💥 قام ${interaction.user} بضربة مضادة ضد <@${targetId}> (+${pts} نقطة)!\n\n` +
              `📊 **النتيجة الحالية:**\n` +
              `🚔 **الشرطة**: \`${warEntry.policeScore}\` نقطة\n` +
              `🔫 **${gangName}**: \`${warEntry.gangScore}\` نقطة`
            )
      ...
      if (action === "manual" && parts[2] === "end") {
        ...
        const warEntry = Object.values(db.wars).find(w => w.gangId === gangId && w.status === "active");
        if (!warEntry)
          return interaction.reply({ content: "❌ لا توجد حرب نشطة.", ephemeral: true });
